...
export default function PendientesTab() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setArticulos(data.map(a => ({ ...a, Intentos: a.Intentos ?? 0 })));
  ...
  const handleFeedbackSubmit = async (comment: string): Promise<void> => {
    ...
    const intentos = (articulo?.Intentos ?? 0) + 1;

    const body: any = {
      Comentario: comment,
      FechaRevision: true,   // se maneja con NOW() en el backend
      Estatus: 2,
      Intentos: intentos
    };
    ...
    toast.success("Artículo rechazado con feedback");
    setArticulos(prev =>
      prev.map(a =>
        a.idArticulo === feedbackModal.articleId
          ? { ...a, Comentario: comment, FechaRevision: new Date().toISOString(), Estatus: 2, Intentos: intentos }
          : a
      )
    );
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden lg:table-cell text-center">Intentos</TableHead>
                ...
                {currentItems.length ? currentItems.map(art => (
                  <TableRow key={art.idArticulo}>
                    ...
                    <TableCell className="hidden lg:table-cell text-center">
                      <Badge variant={art.Intentos! >= 2 ? "destructive" : "default"}>
                        {art.Intentos} / 2
                      </Badge>
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleRejectArticle(art)} className="h-8 w-8 p-0 text-red-600" disabled={art.Intentos! >= 2}><X className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                )) : <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No hay artículos pendientes de revisión</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        </CardContent>
